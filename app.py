...
def generate_app(get_processed_inputs, inpaint):
    ...
    def run_sam():
        ...
        if input_image is None:
            raise gr.Error(
                "No points provided. Click on the image to select the object to segment with SAM"
            )
        ...
        try:
            ...
            return (
                ...
                [(res_mask, "background"), (~res_mask, "subject")],
            )
    ...
    with gr.Blocks() as demo:
        ...
        # Events
        display_img.select(
            get_points, inputs=[display_img], outputs=[sam_mask, display_img]
        )
        ...
        with gr.Row():
            ...
            checkbox = gr.Checkbox(label="Infill subject instead \nof background")
        ...
        submit_inpaint.click(
            fn=run,
            inputs=[prompt, neg_prompt, cfg, random_seed, checkbox],
            outputs=[result],
        )
